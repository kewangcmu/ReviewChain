...
# listen for new events(new unconfirmed review enter unconfirmed section) created by review system
# process the events in a set INTERVAL

# input: eth_account_index (notary account), Watch interval, Starting block 
...
# Load configurations in reviewSysConfig.py
from reviewSysConfig import contract_source_code, contract_address, w3

# Load configurations in supplyChainConfig.py
from supplyChainConfig import supply_source_code, scontract_address, sw3
...
if __name__ == '__main__':
    ...
    # discover new events with the same phase tester
    threads = []
    for i in range(1):
